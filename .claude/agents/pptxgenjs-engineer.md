...
description: Specialist agent for generating Microsoft PowerPoint presentations using pptxgenjs. Use for: writing PPTX generator scripts, debugging pptxgenjs errors, modular section architecture, design system implementation, slide layout code. Use PROACTIVELY whenever pptxgenjs code is being written or debugged.
...
function sectionHeader(s, tag, title, tagColor) {
  // tag: "SECTION|SUBTITLE" — splits on | for two pills
  // Creates top accent bar, tag pill(s), slide title
...
function addCard(s, x, y, w, h_val, fillColor) {
  // Simple rectangle shape. fillColor defaults to "FFFFFF"
...
function sectionHeader(s, tag, title, tagColor) { /* see above */ }
function bottomBar(s, text, y) { /* see above */ }
function addCard(s, x, y, w, h_val, fillColor) { /* see above */ }
